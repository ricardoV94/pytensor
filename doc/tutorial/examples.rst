...
The API is documented in :ref:`pytensor.tensor.random<libdoc_tensor_random_basic>`.
...
You can reseed a shared RNG by setting its value to a fresh generator:

.. testcode::

    rng.set_value(np.random.default_rng(42))

This resets the RNG state so subsequent calls to ``f()`` will produce a new
deterministic sequence.

Other Random Distributions
--------------------------

There are :ref:`many distributions available <libdoc_tensor_random_basic>`,
accessible both as module-level functions (e.g. ``pt.random.normal``) and as
methods on RNG variables (e.g. ``rng.normal()``).
