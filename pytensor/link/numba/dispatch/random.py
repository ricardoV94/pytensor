...
def copy_numba(x):
    ...
    if isinstance(x, types.NumPyRandomGeneratorType):
        def copy_fn(x):
        ...
        raise NotImplementedError(f"Copy overload not implemented for type {type(x)}")

    return copy_fn


@numba_funcify.register(aer.RandomVariable)
def test(op, node, **kwargs):

    if op.ndim_supp > 0:
        raise NotImplementedError("Multivariate RandomVariables not yet supported in Numba backend")

    size_len = get_vector_length(node.inputs[1])
    out_dtype = node.outputs[1].type.dtype
    inplace = op.inplace
    dist_name = op.name

    @numba_basic.numba_njit
    def sample_fn(rng, size, dtype, *params):
        if not inplace:
            rng = copy(rng)
        ...
        if size_len:
            ...
            size = np.broadcast_shapes(*[p.shape for p in params])

        bcast_params = [np.broadcast_to(p, size) for p in params]
        samples = np.empty(size, dtype=out_dtype)
        for i in np.ndindex(size):
            samples[i] = getattr(rng, dist_name)(*[bcast_param[i] for bcast_param in bcast_params])
        return rng, samples
