...
class _ShapeOfProxy:
    """Dict-like proxy so ``shape_feature.shape_of[var]`` works lazily."""

    def __init__(self, feature):
        self._feature = feature

    def __getitem__(self, var):
        return self._feature.shape_tuple(var)
...
class ShapeFeature(Feature):
    r"""Lazy `Feature` that provides shape information via ``get_shape``.

    Shapes are derived on demand by calling ``Op.infer_shape`` on the
    current (live) node inputs — never cached across graph mutations.
    This prevents stale variable references from being reintroduced
    when intermediate nodes are replaced (e.g. xtensor lowering).
    """
    ...
    def __init__(self):
        ...
        self._shape_i_cache: weakref.WeakKeyDictionary = weakref.WeakKeyDictionary()
        # var -> tuple of (shape_expr | None) per dim, transferred from a replaced predecessor
        self._shape_override: weakref.WeakKeyDictionary = weakref.WeakKeyDictionary()
    ...
    def _coerce_shape_el(self, s, node):
        ...
        if isinstance(s, int | np.integer):
            if int(s) < 0:
                raise ValueError(
                    f"infer_shape for {node.op} returned a negative shape: {int(s)}"
                )
    ...
    def _get_node_shapes(self, node):
        ...
        for k, out in enumerate(node.outputs):
            ...
            for j, s in enumerate(sh):
                coerced.append(self._coerce_shape_el(s, node))
    ...
    def get_shape(self, var, idx):
        ...
        if sh is not None:
            s = sh[idx]
            # If infer_shape gave a real answer (not just Shape_i of ourselves), use it
            if not (
                s.owner and isinstance(s.owner.op, Shape_i) and s.owner.inputs[0] is var
            ):
                return s

        # Fall back to transferred shape from a replaced predecessor
        override = self._shape_override.get(var)
        if override is not None and override[idx] is not None:
            return override[idx]
    ...
    @property
    def shape_of(self):
        """Lazy dict-like access for backward compatibility."""
    ...
    def _try_transfer_shape(self, old_var, new_var):
        """Transfer shape dims from old_var to new_var where valid."""
        old_node = old_var.owner
        if old_node is None:
            return
        new_node = new_var.owner

        # Use override from a previous transfer if available, otherwise infer
        old_shape = self._shape_override.get(old_var)
        if old_shape is None:
            node_shapes = self._get_node_shapes(old_node)
            out_idx = old_node.outputs.index(old_var)
            old_shape = node_shapes[out_idx]
        if old_shape is None:
            return

        old_inputs = set(old_node.inputs)
        new_inputs = set(new_node.inputs)

        transferred = []
        any_kept = False
        for s in old_shape:
            # Keep this dim if every old-node input it depends on is also a new-node input
            deps = old_inputs & set(ancestors([s]))
            if deps <= new_inputs:
        ...
        if any_kept:
            self._shape_override[new_var] = tuple(transferred)
    ...
    def on_change_input(self, fgraph, node, i, r, new_r, reason):
        ...
        # Transfer shape knowledge when the replacement op lacks infer_shape
        if (
            hasattr(r.type, "ndim")
            and new_r not in self._shape_override
            and new_r.owner is not None
            and getattr(new_r.owner.op, "infer_shape", None) is None
        ):
            self._try_transfer_shape(r, new_r)
...
def local_shape_to_shape_i(fgraph, node):
    if isinstance(node.op, Shape):
        ...
        ret = as_tensor_variable(
            shape_feature.shape_tuple(node.inputs[0]), ndim=1, dtype="int64"
        )
