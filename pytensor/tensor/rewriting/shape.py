...
def local_shape_to_shape_i(fgraph, node):
    if isinstance(node.op, Shape):
        ...
        v = node.inputs[0]
        shape_dims = [
            shape_feature.get_shape_no_cycle(v, i) for i in range(v.type.ndim)
        ]
        ret = as_tensor_variable(shape_dims, ndim=1, dtype="int64")
...
def local_track_shape_i(fgraph, node):
    ...
    if node in shape_feature.scheduled:
        ...
        return [shape_feature.get_shape_no_cycle(replacement, node.op.i)]
    ...
    i = node.op.i
    new_shape = shape_feature.get_shape_no_cycle(v, i)
