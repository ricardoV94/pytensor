    ...
    * ``x[a:b:-1][::-1]`` -> forward slice, restricted to non-negative
    ...
    def _const_int_or_none(v):
        if v is None:
            return None
        try:
            return int(get_scalar_constant_value(v, max_recur=2))
...
def _merge_scalar_into_slice_unsafe(inner_slice, scalar_index, dim, xshape):
    ...
    def _eager_lt_0(x):
        ...
        if _is_provably_non_negative(x):
            return False
        try:
            return int(get_scalar_constant_value(x, max_recur=4)) < 0
        except NotScalarConstantError:
            return lt(x, 0)
    ...
    step_val = 1 if inner_slice.step is None else _try_const(inner_slice.step)
    ...
    if step_val == 1:
        # x[±a:±b][±idx]
        # Positive idx counts from effective start ∈ [0, ∞).
        # Negative idx counts from effective stop ∈ (-∞, n].
        if start is None:
    ...
    if step_val == -1:
        # x[±a:±b:-1][±idx]
        # Positive idx counts from effective start ∈ (-∞, n-1].
        # Negative idx counts from effective stop ∈ [-1, ∞).
    ...
    if not allow_symbolic_refs:
        ...
        def _is_const(val):
            ...
            if isinstance(val, Constant):
                return True
            try:
                get_scalar_constant_value(val, max_recur=4)
                return True
            except NotScalarConstantError:
                return False

        if not _is_const(len1) or not _is_const(len2):
            return None
        for comp in (slice1.start, slice1.stop, slice1.step):
            if not _is_const(comp):
                return None
        if isinstance(slice2, slice):
            if not all(_is_const(c) for c in (slice2.start, slice2.stop, slice2.step)):
                return None
        elif not _is_const(slice2):
            return None
