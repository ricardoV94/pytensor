...
def test_vectorize_advanced_inc_subtensor1():
    x = vector("x")
    y = dscalar("y")
    idx = ivector("idx")
    ...
    batch_x = matrix("batch_x")
    batch_y = dscalar("batch_y")
    batch_idx = ivector("batch_idx")

    batched_node = _vectorize_node(op, result.owner, batch_x, batch_y, batch_idx)

    assert batched_node.op.idx_list == (slice(None), 0)
...
def test_subtensor_hash_and_eq():
    ...
    s_mix1 = Subtensor(idx_list=[1, slice(None), slice(None)])
    s_mix2 = Subtensor(idx_list=[1, slice(None), slice(None)])
